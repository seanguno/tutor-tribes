...
# Now implement the "up to 3 matches per tutor" logic
# Each tutor can appear in up to 3 other tutors' match lists
# Each tutor shows only their top 3 matches
...
for person, _ in tutor_priorities:
    ...
    # Look for available tutors (assigned less than 3 times)
    for overlap in data['all_overlaps']:
        potential_match = overlap['person']
        
        # Check if this tutor is available (assigned less than 3 times)
        if tutor_assignments[potential_match] < 3:
            available_matches.append(overlap)
    
    # Take top 3 available matches (or fewer if not enough available)
    top_matches = available_matches[:3]
    
    # Mark these tutors as assigned (increment their assignment count)
    ...
    final_results[person] = {
        ...
        'assignments_used': sum(tutor_assignments[match['person']] for match in top_matches)
    }
...
for person in sorted(final_results.keys()):
    ...
    for i, match in enumerate(data['top_matches'], 1):
        # Get the common classes by finding intersection of both tutors' subjects
        common_classes = []
        if match['intersection_size'] > 0:
            # We need to find which specific classes are in common
            # Since we don't have the full subject lists here, we'll show the count and note
            common_classes = [f"{match['intersection_size']} classes in common"]
        else:
            common_classes = ['None']
...
csv_df.to_csv('tutor_overlap_analysis_v7.csv', index=False)

# Create a simple text version for maximum accessibility
text_content = "TUTOR SUBJECT OVERLAP ANALYSIS (V7 - Up to 3 Matches Per Tutor)\n"
text_content += "=" * 75 + "\n\n"
...
with open('tutor_overlap_simple_v7.txt', 'w', encoding='utf-8') as f:
...
<head>
    ...
    <title>Tutor Subject Overlap Analysis (V7 - Up to 3 Matches Per Tutor)</title>
    <style>
        body {
...
html_content += """
        ...
        <div class="main-content" id="main-content">
            <div class="header">
                <h1>📚 Tutor Subject Overlap Analysis (V7)</h1>
...
with open('tutor_overlap_report_v7.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print("✅ V7 analysis with 'up to 3 matches per tutor' logic completed!")
print("📁 Files generated:")
print("   • tutor_overlap_analysis_v7.csv (Enhanced CSV with up to 3 matches per tutor)")
print("   • tutor_overlap_simple_v7.txt (Simple text version)")
print("   • tutor_overlap_report_v7.html (Enhanced HTML with up to 3 matches per tutor)")
print("\nKey changes:")
print("• Each tutor can appear in up to 3 other tutors' match lists")
print("• Each tutor shows only their top 3 matches")
print("• Better balance between coverage and preventing overuse")
print("• Priority system: Tutors with better matches get assigned first")
print("• Shows assignments used and available matches")
...
print("• Allows up to 3 assignments per tutor")
print("• Limits each tutor to top 3 matches")
print("• Prevents excessive overuse while maintaining good coverage")
