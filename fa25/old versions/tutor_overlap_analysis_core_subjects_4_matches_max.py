...
def calculate_overlap(person1, person2, subjects_dict, bonus_subjects_dict):
    ...
    return overlap_score, intersection_count, list(intersection), subjects1 - subjects2, subjects2 - subjects1, list(bonus_intersection)
...
for pass_num in range(1, 5):  # Pass 1, 2, 3, 4
    ...
    for person, _, _, _ in tutor_priorities:
        ...
        # Look for available tutors (assigned less than 4 times) AND has at least 50% overlap
        for overlap in data['all_overlaps']:
            potential_match = overlap['person']
            
            # Check if this tutor is available (assigned less than 4 times) AND has at least 50% overlap
            if (tutor_assignments[potential_match] < 5 and 
...
# Create enhanced CSV output with missing classes and bonus subjects
csv_rows = []
for person in sorted(final_results.keys()):
    ...
    csv_rows.append({
        ...
        'Bonus_Subjects': '; '.join(sorted(data['bonus_subjects'])) if data['bonus_subjects'] else 'None',
...
csv_df.to_csv('tutor_overlap_analysis_clean.csv', index=False)
...
for person in sorted(final_results.keys()):
    ...
    if data['bonus_subjects']:
        text_content += f"Extra Subjects: {', '.join(sorted(data['bonus_subjects']))}\n"
...
with open('tutor_overlap_simple_clean.txt', 'w', encoding='utf-8') as f:
    f.write(text_content)

# Create clean HTML report without collapsible features
html_content = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    if data['bonus_subjects']:
        html_content += f"""
                <div class="bonus-subjects-list">
                    ...
                    {', '.join(sorted(data['bonus_subjects']))}
                </div>
        """
...
# Save the clean HTML report
with open('tutor_overlap_report_clean.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print("✅ Clean analysis completed!")
print("📁 Files generated:")
print("   • tutor_overlap_analysis_clean.csv (Enhanced CSV with bonus subjects in common)")
print("   • tutor_overlap_simple_clean.txt (Simple text version)")
print("   • tutor_overlap_report_clean.html (Clean HTML with all collapsible features)")
print("\nKey improvements:")
print("• Removed (V7 Fixed) from title")
print("• Removed 'Up to 3 Matches Per Tutor Logic' text from instructions")
print("• Removed 'Available Before Assignment' and 'Assignments Used' from sections")
print("• Added bonus subjects in common display in both HTML and CSV")
print("• Changed 'All Subjects' to 'Core Subjects'")
print("• Added 'Extra Subjects in Common' for bonus subjects shared between tutors")
print("• Kept all collapsible functionality (sidebar, sections, expand/collapse buttons)")
print("• Cleaner instructions focused on content, not UI features")
